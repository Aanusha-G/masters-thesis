...
f0_temp<-F0_V2
data<-data.frame(f0_temp, singleton ,voicing)
aov = aov(f0_temp~singleton*voicing,data=data)
summary(aov)

plot_data<-data
plot_data$mean <- with(plot_data,ave(f0_temp,singleton,voicing,FUN=mean))
plot.model <- lm(f0_temp ~ voicing, data=plot_data)

outcome <- predict(plot.model,se.fit=TRUE)
plot_data$CI.half <- outcome$se / 2
...
ggplot(data=summary.plotdata, aes(x=Voicing, y=Means, group=Gemination,color=Gemination,shape=Gemination)) +
  ...
  ggtitle("Variation of F0 of V2 for singletons and geminates")
