...
gems$mean <- with(gems,ave(V1_Dur,poa,voicing,FUN=mean))
sings$mean <- with(sings,ave(V1_Dur,poa,voicing,FUN=mean))
gems.model <- lm(V1_Dur ~ poa*voicing, data=gems)
sings.model <- lm(V1_Dur ~ poa*voicing, data=sings)
...
require(gridExtra)
grid.arrange(a,b, ncol=2, main=textGrob("Interaction effect of POA, voicing and gemination on V1 Duration",gp=gpar(fontsize=15,font=2)))