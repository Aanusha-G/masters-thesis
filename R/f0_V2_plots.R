...
for (i in 1:n){
  ...
  if (sing==F & voiced == T ){
    gem_f0 = append(gem_f0,F0_V2[i],after=length(gem_f0))
    gem_vs_f0 = append(gem_vs_f0,F0_V2[i],after=length(gem_vs_f0))
  }
  else if (sing==F & voiced == F ){
    gem_f0 = append(gem_f0,F0_V2[i],after=length(gem_f0))
    gem_vls_f0 = append(gem_vls_f0,F0_V2[i],after=length(gem_vls_f0))
  }
  else if (sing== T & voiced == F ){
    sing_f0 = append(sing_f0,F0_V2[i],after=length(sing_f0))
    sing_vls_f0 = append(sing_vls_f0,F0_V2[i],after=length(sing_vls_f0))
  }
  else {
    if (sing == T & voiced == T ){
      sing_f0 = append(sing_f0,F0_V2[i],after=length(sing_f0))
      sing_vs_f0 = append(sing_vs_f0,F0_V2[i],after=length(sing_vs_f0))
    }
  }
}
sing_vls_f0<- sing_vls_f0[!is.na(sing_vls_f0)]
