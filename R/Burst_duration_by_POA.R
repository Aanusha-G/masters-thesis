...
for (j in POA_list){
  for (i in 1:n){
    ...
    if (sing==F & voiced == T & POA[i]==j){
      burst_gem_vd = append(burst_gem_vd,Burst.Duration[i],after=length(burst_gem_vd))
    }
    else if (sing==F & voiced == F & POA[i]==j){
      burst_gem_vls = append(burst_gem_vls,Burst.Duration[i],after=length(burst_gem_vls))
    }
    else if (sing==T & voiced == F & POA[i]==j){
      burst_sing_vls = append(burst_sing_vls,Burst.Duration[i],after=length(burst_sing_vls))
    }
    else {
      if (sing==T & voiced == T & POA[i]==j){
        burst_sing_vd =  append(burst_sing_vd,Burst.Duration[i],after=length(burst_sing_vd))
      }
    }
  }
  print (j)
  ...
  cat("Means: VLS, VS",mean(burst_gem_vls),mean(burst_gem_vd))
}